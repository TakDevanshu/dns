...
const { User, DNSRecord, Zone, AuditLog } = require("./models");
...
// Create DNS record (admin only) - ensure zone has default name servers if new
app.post("/domains/create", verifyToken, async (req, res) => {
  try {
    ...
    const { domain, type, name, value, ttl, priority, comment, userId } =
      req.body;
...
app.get("/zones/:domain/nameservers", verifyToken, async (req, res) => {
  try {
    const zone = await Zone.findOne({
      where: { domain: req.params.domain, userId: req.user.userID },
    });
    if (!zone) {
      return res
        .status(404)
...
app.put("/zones/:domain/nameservers", verifyToken, async (req, res) => {
  try {
    ...
    if (!Array.isArray(nameServers) || nameServers.length < 1) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid name servers" });
    }
    const zone = await Zone.findOne({
      where: { domain: req.params.domain, userId: req.user.userID },
    });
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update name servers" });
  }
});
...
// Endpoint to update name servers for a zone
app.put("/zones/:domain/nameservers", verifyToken, async (req, res) => {
  try {
    const { nameServers } = req.body;
    const zone = await Zone.findOne({
...
app.put("/domains/:id", verifyToken, async (req, res) => {
  try {
    ...
    const oldRecord = { ...record.toJSON() }; // <-- Add this line before updating fields

    // Validate updates
    if (
      value &&
      !validateRecordValue(record.type, value, name || record.name)
    ) {
      return res.status(400).json({
        success: false,
